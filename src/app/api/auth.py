...
class AuthResponse(BaseModel):
    ...
    user: UserRead = Field(description="인증된 사용자 정보")
...
@router.post(
    ...
    responses={
        201: {"description": "회원가입 성공. 액세스 토큰과 사용자 정보 반환"},
        400: {"description": "잘못된 요청 - 이메일/사용자명 중복 또는 유효하지 않은 데이터"},
...
async def register(
    ...
    **처리 과정:**
    ...
    3. 로컬 데이터베이스에 사용자 정보 저장
    4. 액세스 토큰 및 리프레시 토큰 발급

    **주의사항:**
    - 이메일은 Supabase에서 고유해야 합니다
    - 사용자명은 로컬 데이터베이스에서 고유해야 합니다
...
@router.post(
    ...
    responses={
        200: {"description": "로그인 성공. 액세스 토큰과 사용자 정보 반환"},
        401: {"description": "인증 실패 - 이메일 또는 비밀번호가 잘못됨"},
        404: {"description": "사용자를 찾을 수 없음 - Supabase에는 있지만 로컬 DB에 없는 경우"},
...
@router.get(
    ...
    responses={
        200: {"description": "현재 사용자 정보 반환 성공"},
        401: {"description": "인증 실패 - 유효한 토큰이 필요함"},
    },
)
